	...
	:BaseResource()
{
	...
	TransitionMain(_cmdList->GetGraphicList(), D3D12_RESOURCE_STATE_DEPTH_WRITE);
}

void DepthStencil::Init(ID3D12Device* _device, DescriptorHandle _handle, CommandList& _cmdList, UINT _width, UINT _height, DXGI_FORMAT _format)
{
	D3D12_RESOURCE_DESC rDesc = {};
	rDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
	rDesc.Width = _width;
...
void DepthStencil::InitStatic(ID3D12Device* _device, CommandList* _cmdList, void* _initData, int _numBytes, const std::wstring& _mainResourceName)
{
	throw; // Not implemented yet
}

void DepthStencil::InitDynamic(ID3D12Device* _device, CommandList* _cmdList, void* _initData, int _numBytes, bool _trippleBuffered, const std::wstring& _mainResourceName)
{
	throw; // Not implemented yet
}
