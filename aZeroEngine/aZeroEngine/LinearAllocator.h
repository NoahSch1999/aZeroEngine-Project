...
namespace aZeroAlloc
{
	...
	*/
	struct AllocationRange
	{
		...
		AllocationRange(UINT startIndex, UINT endIndex)
	...
	*/
	...
	public:
		...
		@return UINT
		*/
		UINT currentOffset() const { return m_currentIndex; }
	};
	...
	*/
	...
	public:
		...
		*/
		LinearUploadAllocator(ID3D12Device* device, UINT maxElements, UINT numSubresources)
		...
		*/
		...
		@return Allocation<T>
		*/
		Allocation<T> getAllocation(UINT numElements)
		{
			UINT startIndex = m_currentElement;
			UINT endIndex = m_currentElement + numElements;
	...
	*/
	...
	public:
		...
		@param numSubresources Number of subresources. Should be the same as frames in flight
		*/
		LinearDefaultAllocator(ID3D12Device* device, UINT maxElements, UINT numSubresources)
		...
		@return void
		*/
		void updateRange(ID3D12GraphicsCommandList* commandList, AllocationRange range, UINT frameIndex)
		{
			...
			UINT64 numElementsToCopy = (range.m_endIndex - range.m_startIndex) * sizeof(T);

			commandList->CopyBufferRegion(this->m_defaultBuffer.Get(), defaultOffset, this->m_uploadBuffer.Get(), uploadOffset, numElementsToCopy);
		}
		...
		@return void
		*/
		void updateAll(ID3D12GraphicsCommandList* commandList, UINT frameIndex)
		{
			UINT64 uploadOffset = (sizeof(T) * this->m_maxElements) * frameIndex;
			commandList->CopyBufferRegion(this->m_defaultBuffer.Get(), 0, this->m_uploadBuffer.Get(), uploadOffset, this->m_maxElements);
		}
