...
void Texture2D::Init(ID3D12Device* _device, CommandList& _transitionList, CommandList& _copyList, 
	...
	UINT _width, UINT _height, UINT _bytePerTexel, DXGI_FORMAT _format, const std::string& _name)
{
	SetName(_name);

	TextureResource::Init(_device, _uploadResource, _transitionList, _copyList, _data, _width, _height, _bytePerTexel, _format, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
	handle = _srvHandle;
