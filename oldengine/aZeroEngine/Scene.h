...
public:
	...
	*/
	...
	aZeroECS::Entity& DuplicateEntity(aZeroECS::Entity srcEntity)
	{
		std::string entityName(this->GetEntityName(srcEntity).value());
		entityName += "_C";
		aZeroECS::Entity& newEntity = this->CreateEntity(entityName);

		Transform* srcTransform = m_componentManager->GetComponent<Transform>(srcEntity);
		Transform* destTransform = m_componentManager->GetComponent<Transform>(newEntity);
		*destTransform = *srcTransform;

		Mesh* srcMesh = m_componentManager->GetComponent<Mesh>(srcEntity);
		if (srcMesh)
		{
			AddComponentToEntity<Mesh>(newEntity, std::move(*srcMesh));
		}

		MaterialComponent* srcMaterial = m_componentManager->GetComponent<MaterialComponent>(srcEntity);
		if (srcMaterial)
		{
			AddComponentToEntity<MaterialComponent>(newEntity, std::move(*srcMaterial));
		}

		PointLightComponent* srcPointLightComp = m_componentManager->GetComponent<PointLightComponent>(srcEntity);
		if (srcPointLightComp)
		{
			PointLight* pLight = lSystem->GetLight(*srcPointLightComp);
			AddComponentToEntity<PointLightComponent>(newEntity);
			PointLightComponent* destPointLightComp = m_componentManager->GetComponent<PointLightComponent>(newEntity);
			PointLight* destLight = lSystem->GetLight(*destPointLightComp);
			*destLight = *pLight;
		}
